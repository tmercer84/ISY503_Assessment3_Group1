...
def random_shadow(image):
    ...
    # Set mask elements based on the shadow region
    mask[(ym - y1) * (x2 - x1) - (y2 - y1) * (xm - x1) > 0] = 1

    # Apply shadow by reducing the brightness of the masked region
    cond = mask == np.random.randint(2)
    s_ratio = np.random.uniform(low=0.2, high=0.5)

    # Apply shadow to each color channel separately
    for channel in range(image.shape[2]):
        image[:, :, channel][cond] = image[:, :, channel][cond] * s_ratio
